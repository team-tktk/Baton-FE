  ...
  return (
    <section className={styles.wrapper}>
      ...
      <article className={styles.card}>
        ...
        <footer><button className={styles.back} disabled={currentStep === 1} type="button" onClick={onBack}><Icon name="back" /> 이전 질문</button><div><button className={styles.skip} type="button" onClick={onSkip}>질문 건너뛰기</button><button className={styles.submit} type="button" onClick={submit}>{currentStep === total ? '답변 반영하고 초안 보기' : '다음 질문'} <Icon name="arrow" /></button></div></footer>
      </article>
    </section>
  )
}
