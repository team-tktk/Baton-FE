    ...
    <section className={styles.section}>
      ...
      <div className={styles.list}>
        {items.map((item, index) => (
          <label key={index}><span>{index + 1}</span><input placeholder="업무를 입력하세요" value={item} onChange={(event) => onChange(index, event.target.value)} /><button aria-label={`${index + 1}번 업무 삭제`} type="button" onClick={() => onRemove(index)}>×</button></label>
        ))}
      </div>
