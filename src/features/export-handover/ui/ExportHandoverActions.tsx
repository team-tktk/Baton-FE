...
export function ExportHandoverActions({ handover, onFeedback, compact = false }: ExportHandoverActionsProps) {
  ...
  return <div className={styles.actions}>
    ...
    <Button aria-label={compact ? '파일 다운로드' : undefined} variant="secondary" onClick={() => { downloadMarkdown(markdown, handoverMarkdownFilename); onFeedback('Markdown 파일을 내려받았어요') }}><Icon name="download" />{compact ? null : ' 파일 다운로드'}</Button>
  </div>
}
