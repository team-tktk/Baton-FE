...
export function HandoverDraftEditor(props: HandoverDraftEditorProps) {
  ...
  const renderTasks = (items: HandoverTask[]) => items.map((task) => {
    ...
    return <article className={styles.task} key={task.id}>
      ...
      <ul>
        ...
        {task.nextAction && <li>{edit(`${task.title} 다음 할 일`, `task.${task.id}.nextAction`, `다음 할 일: ${task.nextAction}`)}</li>}
  ...
  return <article className={styles.editor}>
    <section className={styles.page}>
      ...
      {document.activeTasks.length > 0 && <section className={styles.section}><h2>진행 중인 업무</h2><p>먼저 이어서 해야 할 일입니다.</p>{renderTasks(document.activeTasks)}</section>}
      {document.recurringTasks.length > 0 && <section className={styles.section}><h2>반복 업무</h2><p>정해진 주기에 맞춰 진행합니다.</p>{renderTasks(document.recurringTasks)}</section>}
      {document.criteria.length > 0 && <section className={styles.section}><h2>업무 기준과 예외</h2><p>담당자가 바뀌어도 같은 판단을 내리기 위한 기준입니다.</p><ul className={styles.bullets}>{document.criteria.map((criterion) => <li key={criterion.id}>{edit(`${criterion.title} 내용`, `criterion.${criterion.id}`, criterion.defaultText)}</li>)}</ul></section>}
      {peopleRows.length > 0 && <section className={styles.section}><h2>주요 관계자</h2><p>업무별로 도움을 받을 사람입니다.</p><MockTable headers={['이름', '소속', '도움을 받을 내용']} rows={peopleRows} /></section>}
      {toolRows.length > 0 && <section className={styles.section}><h2>사용 도구와 자료</h2><MockTable headers={['파일', '용도']} rows={toolRows} /></section>}
      {document.schedule.length > 0 && <section className={styles.section}><h2>업무 일정</h2><p>반복 시점과 완료 기준을 함께 확인합니다.</p><MockTable headers={['주기', '업무', '완료 기준']} rows={document.schedule.map((row) => [row.cycle, row.task, row.detail])} /></section>}
      {document.accessAccounts.length > 0 && <section className={styles.section}><h2>접근 권한과 계정</h2><p>업무 시작 전에 필요한 시스템 권한입니다.</p><MockTable headers={['도구', '필요 권한', '상태']} rows={document.accessAccounts.map((row) => [row.tool, row.permission, row.status])} /></section>}
      {document.checklist.length > 0 && <section className={styles.section}><h2>첫 주 체크리스트</h2><ul className={styles.checklist}>{document.checklist.map((item, index) => <li key={`${item}-${index}`}><span>□</span>{edit(`체크리스트 ${index + 1}`, `checklist.${index}`, item)}</li>)}</ul></section>}
    </section>
