...
export function HandoverChat({ handoverId }: { handoverId: string }) {
  ...
  return <section className={styles.chat}>
    ...
    <div className={styles.messages} aria-live="polite">{messages.map((message) => <article className={message.role === 'user' ? styles.user : styles.assistant} key={message.id}><p><AnswerText text={message.text} /></p>{message.citations?.map((citation) => <span key={citation.sourceId}><Icon name="link" /> {[citation.title, citation.locator].filter(Boolean).join(' · ')}</span>)}</article>)}{status === 'sending' && <article className={styles.assistant}><p className={styles.typing}>자료에서 답을 찾고 있어요…</p></article>}{status === 'error' && <p className={styles.error}>답변을 불러오지 못했어요. 잠시 후 다시 질문해 주세요.</p>}<div ref={endRef} /></div>
