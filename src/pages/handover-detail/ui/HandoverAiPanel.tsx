 ...
 */
export function HandoverAiPanel({ handoverId, hidden, onClose, panelRef }: {
  handoverId: string
  hidden?: boolean
  onClose: () => void
  ...
  useEffect(() => { if (!hidden) endRef.current?.scrollIntoView({ behavior: 'smooth', block: 'nearest' }) }, [hidden, messages, status])

  return <aside aria-labelledby="handover-ai-title" className={styles.aiPanel} hidden={hidden} ref={panelRef} tabIndex={-1}>
