...
type Filter = 'pending' | 'revision' | 'approved'
...
const matches = (item: ReviewSummary, filter: Filter) => filter === 'pending'
  ? item.status === 'submitted'
  : filter === 'revision' ? item.status === 'revision-requested' : item.status === 'approved'
...
export function ReviewInboxPage() {
  ...
  const filter: Filter = tabs.some((tab) => tab.filter === requested) ? requested as Filter : 'pending'
  ...
  const countOf = (target: Filter) => reviews?.filter((item) => matches(item, target)).length ?? 0
  return <>
    <button className={styles.homeBack} type="button" onClick={() => navigate('/')}><Icon name="back" /> 홈으로</button>
    <main className={styles.main}>
      <header><div><span><Icon name="check" /> 인수인계 확인하기</span><h1>검토할 인수인계</h1><p>제출된 문서를 확인하고 승인하거나 보완 의견을 남기세요.</p></div><div className={styles.pendingCount}><strong>{countOf('pending')}</strong><span>승인 대기</span></div></header>
      {reviews ? <>
        <nav aria-label="검토 상태 필터">{tabs.map((tab) => <button aria-pressed={filter === tab.filter} key={tab.filter} type="button" onClick={() => setParams({ status: tab.filter })}>{tab.label} {countOf(tab.filter)}</button>)}</nav>
