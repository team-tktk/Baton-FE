...
export function ReviewInboxPage() {
  ...
  return <>
    ...
    <main className={styles.main}>
      ...
      {reviews ? <>
        <nav aria-label="검토 상태 필터">{tabs.map((tab) => <button aria-pressed={filter === 'all' ? tab.filter === 'pending' : filter === tab.filter} key={tab.filter} type="button" onClick={() => setParams({ status: tab.filter })}>{tab.label} {countOf(tab.filter)}</button>)}</nav>
